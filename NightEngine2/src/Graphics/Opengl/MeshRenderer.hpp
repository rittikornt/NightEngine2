...
namespace NightEngine::EC::Components
{
  class MeshRenderer: public ComponentLogic
  {
    ...
    public:
      ...
      void InitMesh(const std::vector<NightEngine::Rendering::Opengl::Vertex>& vertices
        , const std::vector<unsigned>& indices
        , bool castShadow = true
        , bool buildNow = true);

      //! @brief Init from mesh information
      void InitMesh(const NightEngine::Rendering::Opengl::Vertex* vertices, size_t vertexArraySize
      ...
      //! @brief Set Material
      void SetMaterial(EC::Handle<NightEngine::Rendering::Opengl::Material> material) { m_material = material; }

      //! @brief Get Material
      NightEngine::Rendering::Opengl::Material* GetMaterial(void) { return m_material.IsValid()?m_material.Get(): nullptr; }
      ...
      //! @brief Get Meshes
      const std::vector<NightEngine::Rendering::Opengl::Mesh>& GetMeshes(void) { return m_meshes; }

      //! @brief Check if casting shadow
      bool IsCastingShadow(void) { return m_castShadow; }

      //! @brief Draw mesh with custom m_material
      void DrawWithMaterial(NightEngine::Rendering::Opengl::ShaderUniformsFn fn = nullptr);

      //! @brief Plain draw loop
      void DrawMeshes(void);

      //! @brief Draw mesh without binding material
      void DrawWithoutBind(bool useTexture, NightEngine::Rendering::Opengl::Shader& shader);

      //! @brief Draw mesh without binding material
      void DrawWithoutBindDepthPass(bool useTexture, NightEngine::Rendering::Opengl::Shader& shader);

      //! @brief Draw mesh based on mode
      void DrawWithMode(bool useTexture, NightEngine::Rendering::Opengl::Shader& shader);
    ...
    private:
      EC::Handle<NightEngine::Rendering::Opengl::Material> m_material;

      std::vector <EC::Handle<NightEngine::Rendering::Opengl::Material>> m_materials;
      bool                            m_useModelLoadedMaterials = false;
      bool                            m_isOpaque = true;

      std::vector<NightEngine::Rendering::Opengl::Mesh>    m_meshes; //All the submeshes
